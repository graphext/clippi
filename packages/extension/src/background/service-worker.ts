...
async function sendToContentScript(message: BackgroundToContentMessage): Promise<void> {
...
async function handleSidePanelMessage(
  ...
  switch (message.type) {
    ...
    case 'UPDATE_TARGET': {
      ...
      if (target) {
        Object.assign(target, message.payload.updates, { updatedAt: Date.now() })
        await saveState()
        broadcastState()
      }
      break
    ...
    case 'UPDATE_STEP': {
      const target = state.targets.find((t) => t.id === message.payload.targetId)
      if (target) {
    ...
    case 'DELETE_STEP': {
      const target = state.targets.find((t) => t.id === message.payload.targetId)
      if (target) {
        target.steps = target.steps.filter((s) => s.id !== message.payload.stepId)
        target.updatedAt = Date.now()
        await saveState()
        broadcastState()
      }
    ...
    case 'IMPORT_MANIFEST': {
      try {
        ...
        if (!imported.targets || !Array.isArray(imported.targets)) {
          return { type: 'ERROR', payload: { message: 'Invalid manifest: missing targets array' } }
        }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (sender.tab) {
    ...
    handleSidePanelMessage(message as SidePanelToBackgroundMessage).then((response) => {
      if (response) {
